...
def load_models():
    ...
    emb_model = BertModel.from_pretrained(
        model_path, local_files_only=True
    ).to(device).eval()
...
def get_churn_label(band):
    ...
    labels = {
        ...
        "KIRMIZI": "Yüksek Riskli (Kırmızı)",
        "SARI": "Orta Riskli (Sarı)",
        "YEŞİL": "Düşük Riskli (Yeşil)"
    }
...
def show_dashboard(df):
    ...
    with k5:
        st.markdown(kpi_style.format(border=colors['KIRMIZI'], icon='🔴', label='Yüksek Riskli (Kırmızı)', color=colors['KIRMIZI'], value=f'{kirmizi:,}', sub=f'%{(kirmizi/n*100) if n > 0 else 0:.1f}'), unsafe_allow_html=True)
    ...
    with col_band:
        ...
        fig_band = go.Figure(data=[go.Pie(
            labels=['Kritik Riskli (Mor)', 'Yüksek Riskli (Kırmızı)', 'Orta Riskli (Sarı)', 'Düşük Riskli (Yeşil)'],
    ...
    if 'Alt_Kategori' in fdf.columns and 'churn_band' in fdf.columns and n > 0:
        ...
        fig = go.Figure()
        fig.add_trace(go.Bar(name='Düşük Riskli (Yeşil)', y=alt_cats_with_unit, x=yesil_vals, orientation='h', marker_color=colors['YEŞİL']))
        fig.add_trace(go.Bar(name='Orta Riskli (Sarı)', y=alt_cats_with_unit, x=sari_vals, orientation='h', marker_color=colors['SARI']))
        fig.add_trace(go.Bar(name='Yüksek Riskli (Kırmızı)', y=alt_cats_with_unit, x=kirmizi_vals, orientation='h', marker_color=colors['KIRMIZI']))
...
def show_complaint_analysis(tokenizer, clf_model, emb_model, device, df):
    ...
    # Örnek metinler
    ornek_baslik = "Teslim Edilmeyen Sipariş ve Yaşadığım Mağduriyet"
    ornek_metin = """Güvenerek alışveriş yaptım ancak siparişim halen teslim edilmedi. İlk kez böyle bir şey başıma geliyor . Defalarca aramama rağmen geri dönüş yapılmadı ve herhangi bir çözüm sunulmadı. Bu süreçte ciddi bir mağduriyet yaşıyorum ve sorun hala devam ediyor. Siparişim 15 gündür kargoda Türkiye turu yaptı benim evime uğramadı sadece 

Daha önce de benzer bir sorun yaşamıştım, ancak yine aynı durumla karşılaştım. Yaşadığım bu durum büyük bir hayal kırıklığı yarattı ve açıkçası pişman oldum. Artık bu sorunun en kısa sürede giderilmesini bekliyorum, aksi halde bir daha alışveriş yapmayı düşünmüyorum. Sorun çözülmediği takdirde tüketici hakem heyetine başvuracağım"""
                    ...
                    try:
                        ...
                        fig = go.Figure(go.Indicator(
                            ...
                            gauge={
                                ...
                                'bar': {'color': color},
                                'steps': [
                                    {'range': [0, 35], 'color': "rgba(34, 197, 94, 0.3)"},  # Düşük Riskli (Yeşil): #22c55e
                                    {'range': [35, 50], 'color': "rgba(250, 204, 21, 0.3)"},  # Orta Riskli (Sarı): #facc15
                                    {'range': [50, 70], 'color': "rgba(220, 38, 38, 0.3)"},  # Yüksek Riskli (Kırmızı): #dc2626
...
# =========================
# 2) "Strong active start" bul (0'a yakın dönemleri ele)
# =========================
